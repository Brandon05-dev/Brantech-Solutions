...
function removeGradients(filePath) {
  ...
  if (filePath.endsWith('.css')) {
    ...
    content = content.replace(/--gradient-primary:.*?;/g, '--gradient-primary: hsl(var(--primary));');
    content = content.replace(/--gradient-secondary:.*?;/g, '--gradient-secondary: hsl(var(--secondary));');
    content = content.replace(/--gradient-hero:.*?;/g, '--gradient-hero: hsl(var(--primary));');
  } else {
